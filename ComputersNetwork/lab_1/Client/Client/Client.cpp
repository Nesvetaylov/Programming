...
void main() {
	...
	const char* nameR = "C:/Users/Администратор/Desktop/Client/Client/REQUEST.bin"; //файл для запросов клиентов
	const char* nameA = "C:/Users/Администратор/Desktop/Client/Client/ANSWER.bin"; //файл для ответов сервера
	ofstream f_REQ;//создать файл запросов
	ifstream f_ANS;//открыть файл ответов
	long pred_size;//хранениn размер предыдущего ответа сервера
	...
	while (true)//начало работы клиента
	{
		...
		pred_size = f_ANS.tellg();//Получение размера предыдущего ответа сервера и сохранение в переменной pred_size
